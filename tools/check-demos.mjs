...
await test('Restaurant · commande au clavier puis devis',async p=>{
 ...
 await activate(p,'[data-dish=pad][data-delta="1"]');await p.key('Enter');await activate(p,'[data-dish=tea][data-delta="1"]');await activate(p,'[data-send-order]');
 assert.deepEqual(await p.evaluate('Array.from(document.querySelectorAll("[data-ticket-items] li")).map(e=>e.textContent)'),['2 × Pad thaï','1 × Thé glacé']);
